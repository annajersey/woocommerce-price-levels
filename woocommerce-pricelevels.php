...
//if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	...
	if ( ! class_exists( 'WC_PriceLevels' ) ) {
		 ...
		 * Localisation
		 **/
		load_plugin_textdomain( 'wc_pricelevels', false, dirname( plugin_basename( __FILE__ ) ) . '/i18n/languages' );
		...
		class WC_PriceLevels {
			...
			public function return_custom_price_level () {
				 global $post;
				...
				//echo $post_id;
				if($role){
					$price = get_post_meta($post_id,$role.'_price' , true);
			...
			function price_for_roles() {
				...
				foreach($all_roles as $key=>$role){
					woocommerce_wp_text_input( array( 'id' => $key.'_price', 'class' => 'wc_input_price short', 'label' => __( $role['name'].' 	Price', 'woocommerce' ) . ' (' . get_woocommerce_currency_symbol() . ')' ) );
				}
			}
			...
			public  function plugin_activate() {
			 if(!in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) )){
                ...
                wp_die(__("WooCommerce Price Levels requires WooCommerce installed and activated.")); 
			  }
			}
			public  function register_my_custom_submenu() {
				add_submenu_page( 'woocommerce', 'Customer Levels', 'Customer Levels', 'manage_options', 'customer-levels', array( 'WC_PriceLevels','customer_levels_page_callback' )   ); 
			...
			public function new_roles_page_callback() {
				...
				<form method="post" action="'.$_SERVER['REQUEST_URI'].'">
					<div id="titlewrap">
					<label for="title" id="title-prompt-text" class="">Role Name</label>
					<input type="text" autocomplete="off" id="title" value="" size="30" name="role_name">
					</div><br />
					<input id="publish" class="button button-primary button-large" type="submit" accesskey="p" value="Save" name="publish">
				</form>';
				}
